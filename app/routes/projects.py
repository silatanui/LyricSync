...
def _forbidden():
    return jsonify({
        "success": False,
        "error": {"code": "FORBIDDEN", "message": "You do not have access to this project.", "retryable": False}
    }), 403

def _can_access(project: Project) -> bool:
    """Projects with no owner remain accessible to anyone (guest/demo projects)."""
    if not project.user_id:
        return True
    return current_user.is_authenticated and current_user.id == project.user_id

@projects_bp.route("", methods=["GET"])
def list_projects():
    """List projects owned by the current user (or unowned/guest projects when logged out)."""
